...
cache("depmap_modelling_df",
      depends = c("gene_effect", "ccle_kras_double_muts", "mapk_muts",
                  ...
                  "gene_is_deleted", "gene_is_mutated", "ccle_expression"),
{
    depmap_modelling_df <- gene_effect %>%
        filter(!dep_map_id %in% c(!!ccle_kras_double_muts$dep_map_id,
        ...
        mutate(is_deleted = ifelse(is.na(is_deleted), FALSE, is_deleted),
      ...
      depends = c("depmap_modelling_df", "essentiality_tib"),
{
    essential_gene_effect_summary <- depmap_modelling_df %>%
        ...
        summarise(avg = mean(gene_effect),
                  sd = sd(gene_effect)) %>%
        ungroup() %>%
        pivot_wider(cancer,
                    names_from = label,
