...
ProjectTemplate::cache("hint_gr", {
  ...
  edge_list_path_1 <- file.path(
    hint_data_path,
    "HomoSapiens_binary_hq.txt"
  )
  edge_list_path_2 <- file.path(
    hint_data_path,
  ...
  depends = c("string_gr", "bioplex_gr", "hint_gr"),
  {
    ...
    print_graph_summary_stats <- function(gr, name) {
      ...
      tibble(
        graph_name = name,
        metric_name = c(
          "num_edges", "num_vertices", "num_multiple",
          "diameter", "num_max_cliques", "mean_distance",
          "transitivity"
        ),
        value = c(
          num_edges, num_vertices, num_mult, gr_diameter,
