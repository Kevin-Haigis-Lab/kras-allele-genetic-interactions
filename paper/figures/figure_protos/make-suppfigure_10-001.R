# Supplemental Figure 10. Labeled high-level view of comutation network for PAAD

FIGNUM <- 10
SUPPLEMENTAL <- TRUE
VERSION <- 1

FIG_DIMENSIONS <- get_figure_dimensions(2, "tall")


#' Special theme for graphs from 'ggraph'.
theme_graph_figS10 <- function() {
    theme_graph(base_size = 7, base_family = "Arial") %+replace%
    theme(
        plot.title = element_blank(),
        plot.tag = element_text(size = 7,
                                face = "bold",
                                margin = margin(0, 0, 0, 0, "mm")),
        legend.margin = margin(0, 0, 0, 0, "mm"),
        legend.position = "bottom",
        legend.title = element_text(size = 5, hjust = 0.5),
        legend.text = element_text(size = 5, hjust = 0.5),
        plot.margin = margin(0, 0, 0, 0, "mm")
    )
}


#### ---- A. Labeled PAAD comutation graph ---- ####
# The comutation graph for PAAD with every node labeled.
# original script: "src/20_40_highlivel-genetic-interactions.R"


panel_A <- read_fig_proto("genetic_interaction_network_labeled_PAAD",
                          FIGNUM,
                          supp = SUPPLEMENTAL) +
    theme_graph_figS10()


#### ---- Figure assembly ---- ####

{
    set.seed(0)
    # COMPLETE FIGURE
    full_figure <- panel_A +
        plot_layout()

    save_figure(
        full_figure,
        figure_num = FIGNUM,
        supp = SUPPLEMENTAL,
        dim = FIG_DIMENSIONS
    )
}
