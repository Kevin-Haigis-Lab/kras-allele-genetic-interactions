...
theme_graph_fig2 <- function(plot_margin = margin(0, 0, 0, 0, "mm")) {
...
panel_B <- read_fig_proto(
  ...
  theme_graph_fig2(plot_margin = margin(0, 0, 0, 0, "mm")) %+replace%
    theme(
      legend.title = element_markdown()
    ) +
  labs(
...
panel_D <- adjust_oncoplot_theme(
  panel_D, c(0, 450), c(50, 200, 400),
  margin(0, 0, 0, -3.5, "mm")
)
...
panel_F <- adjust_oncoplot_theme(
  panel_F, c(0, 1150), c(100, 500, 1000),
  margin(0, 0, 0, -3.5, "mm")
)
