...
plot_distribution_of_mutation_count <- function(cancer,
  ...
  p <- data %>%
    mutate(
      tumor_sample_barcode = fct_reorder(tumor_sample_barcode, n),
...
distribution_kras_vaf <- cancer_coding_muts_df %>%
  ...
  mutate(
    adjusted_vaf = VAF / cancer_dna_fraction,
  ...
  labs(
    x = "*KRAS* mutation VAF (adjusted for tumor sample purity)",
    y = "density"
  )
ggsave_wrapper(
  distribution_kras_vaf,
...
comut_genes_vaf_dist <- cancer_coding_muts_df %>%
  ...
  labs(
    x = "mutation VAF (adjusted for tumor sample purity)",
    y = "density"
  )
ggsave_wrapper(
  comut_genes_vaf_dist,
...
diff_adjusted_vaf_dist <- cancer_muts_adjvaf %>%
  ...
  labs(
    x = "difference in VAF of *KRAS* and comutated gene mutations",
    y = "density"
  )
ggsave_wrapper(
  diff_adjusted_vaf_dist,
...
purity_vs_vaf_scatter <- cancer_muts_adjvaf %>%
  ...
  ungroup() %>%
  mutate(
    upper_VAF = minmax(avg_VAF + sd_VAF, 0, 1),
