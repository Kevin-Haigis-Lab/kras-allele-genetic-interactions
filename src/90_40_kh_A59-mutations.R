...
show_df <- function(df) {
  knitr::kable(df, format = "markdown", digits = 3)
}
...
a59_cancer_data <- cancer_full_coding_muts_df %>%
  ...
  mutate(
    a59_mutation = ifelse(is.na(a59_mutation), "other", a59_mutation),
...
a59_cancer_data %>%
  ...
  summarise(
    in_mapk_kegg_gs = any(in_mapk_kegg_gs),
    in_mapk_spec_gs = any(in_mapk_spec_gs)
  ) %>%
  group_by(is_a59_mutation) %>%
  summarise(
    num_in_mapk_kegg_gs = sum(in_mapk_kegg_gs),
...
a59_mapk_mut_counts <- a59_cancer_data %>%
  ...
  left_join(num_samples_df, by = "grp") %>%
  mutate(
    num_not_mapk_kegg_gs = n - num_in_mapk_kegg_gs,
...
a59_mapk_mut_counts %>%
  group_by(grp) %>%
  summarise(
    frac_mapk_kegg_mut = sum(is_mapk_kegg_mut) / n(),
...
mut_signature_contribution_a59muts <- a59_mut_signatures %>%
  group_by(
    tumor_sample_barcode, is_hypermutant,
