...
mutsig_by_kras_for_T <- matrix(
  rep(mutsig_by_tumor, 4),
  nrow = nrow(mutsig_by_tumor)
) * mutsig_by_kras
...
mutsig_by_kras_noR <- t(apply(
  mutsig_by_kras_noR, 1,
  function(r) {
    r / sum(r)
  }
))
...
message(glue(
  "prob. of KRAS allele 'a' without removing remainder: {round(res1, 4)}"
))
message(glue(
  "   prob. of KRAS allele 'a' with removing remainder: {round(res2, 4)}"
))
