...
simulation_grid <- expand.grid(
  simulation_idx = seq(1, num_simulations),
...
submit_simulation_job_arrays <- function(job_array_idx, data) {
  if (file.exists(simulation_output_file_name(job_array_idx))) {
    return()
  }
...
cache("comutation_fdr_simulation_results", {
  ...
  comutation_fdr_simulation_results <- rep(
    list(NA),
...
pval_plot <- simulation_res_stats %>%
  ...
  scale_x_continuous(
    limits = c(
      min(broad_mutation_rate_seq),
      max(broad_mutation_rate_seq)
    ),
    expand = c(0, 0)
  ) +
  scale_fill_gradient2(
    low = blue_col, high = red_col,
