...
srsf5_box_plot <- srsf5_box_plot_data %>%
  ...
  labs(
    y = glue("dependency score of *{srsf5_res$hugo_symbol[[1]]}*"),
    color = "mutation"
  )
...
fkbp1a_box_plot <- fkbp1a_box_plot_data %>%
  ...
  geom_line(
    data = filter(fkbp1a_box_plot_summary, kras_allele == 0),
    group = "a",
    color = stds$line$grey,
    size = stds$line$size
  ) +
  ...
  geom_line(
    data = filter(fkbp1a_box_plot_summary, kras_allele == 1),
    group = "a",
    color = "#87a7ff",
    size = stds$line$size
  ) +
  ...
  labs(
    y = glue("dependency score of *{fkbp1a_res$hugo_symbol[[1]]}*"),
    color = "*KRAS* allele"
  )
