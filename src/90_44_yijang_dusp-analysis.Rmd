...
\alpha \sim \mathcal{N}(0, 2) \quad \alpha_{DUSP} \sim \mathcal{N}(0, 5) \quad K_{DUSP} \sim \mathcal{N}(0, 2) \\
\sigma \sim \text{Exp}(1)
$$
...
- data frame of APC mutant and RNA expression of DUSPs
- show basic descriptive plots

```{r, fig.height=11, fig.width=8}
apc_dusp_rna_data <- coad_apc_mutations %>%
  group_by(tumor_sample_barcode) %>%
  summarise(apc_mutations = list(mutation_type),
...
apc_mut_pal <- c("TRUE" = "#267BD5", "FALSE" = "#FBA53D")
...
apc_dusp_rna_data %.% {
  ...
  geom_boxplot(aes(color = is_apc_mut), outlier.shape = NA) +
...
apc_dusp_rna_data %.% {
  ...
  geom_density(aes(color = is_apc_mut, fill = is_apc_mut), size = 1.2, alpha = 0.2) +
  scale_x_log10() +
...
- fit model
- check fitting process was fine

From the above density plots, I think it would be best to only model a selection of the *DUSP*s.
At the very least, the *DUSP*s with clear bi-modal distributions should be removed as they will not be modeled by a Gaussian distribution.
